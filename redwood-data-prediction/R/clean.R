...
# A function for cleaning the redwood_orig data
cleanRedwoodData <- function(redwood_df) {
  # Argument:
     # Take in a redwood dataset 
  # Return:
    # Return a cleaned redwood dataframe
  ...
  redwood_df <- redwood_df %>%
    # select voltage in the range 2.2~3 
    # Please see "Data Cleaning" section" for rationale behind this range
    filter(voltage <= 3 & voltage >= 2.2) %>%
    # select humidity from 0 to 100% due to common sense 
    filter(humidity <= 100 & humidity >=0 ) %>% 
    # This condition is under the assumption: incident PAR > refelcted
    # Please see "Data Cleaning" for why the assumption is valid
    filter(hamatop>=hamabot)
